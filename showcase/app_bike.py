...
def build_model(config: dict) -> nn.Module:
    ...
    input_dim, hidden = config['input_dim'], config['hidden']
    model = nn.Sequential(
    ...
    return model                         
    raise ScaffoldIncomplete("TODO 1: 체크포인트 설정으로 회귀 모델 구조를 만드세요.")
...
def load_model():
    ...
    raise ScaffoldIncomplete("TODO 2: 학습된 가중치를 복원하고 추론 모드로 전환하세요.")
...
def prepare_features(values: dict[str, float], checkpoint: dict) -> torch.Tensor:
    ...
    return normalized
    raise ScaffoldIncomplete("TODO 3: 특성 순서 복원과 train 통계 표준화를 완성하세요.")
...
def predict_count(model: nn.Module, normalized: torch.Tensor) -> float:
    ...
    raise ScaffoldIncomplete("TODO 4: 배치 차원·추론 모드·스칼라 변환을 연결하세요.")
...
def apply_page_style() -> None:
    st.markdown(
        ...
        [data-testid="stSidebar"] { background:#fffaf0; border-right:1px solid #decfb8; color:var(--ink)}
